...
:: ==============================================
set "app=%~dp0"
set "CHROMIUM_PATH=%app%brave.exe"
set "BROWSER_NAME=Brave Origin Portable"
set "BROWSER_ID=BraveOriginPortable"
set "BROWSER_DESC=Brave Origin Portable default browser with custom profile"
...
if not exist "%CHROMIUM_PATH%" (
    echo ERROR: Brave Origin Portable not found at:
    echo "%CHROMIUM_PATH%"
    pause
    exit /b 1
)
